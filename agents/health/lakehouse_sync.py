...
class LakehouseSyncMixin:
    ...
    def configure_lakehouse_sync(self, project_id: str, branch_id: str,
                                  source_tables: list[str],
                                  target_catalog: str = "ops_catalog",
                                  target_schema: str = "lakebase_ops") -> dict:
        """
    ...
    def monitor_replication_lag(self, project_id: str, branch_id: str) -> dict:
        ...
        repl_data = self.client.execute_query(
            project_id, branch_id, LAKEHOUSE_SYNC_REPLICATION_LAG
        )
    ...
    def validate_scd_history(self, project_id: str, branch_id: str,
                              target_table: str,
                              key_column: str = "id") -> dict:
        """
        ...
        src_result = self.client.execute_query(
            ...
            f"SELECT COUNT(*) as count FROM {target_table.split('.')[-1].replace('_cdc', '')}"
        )
        ...
        if not all_valid:
            ...
            self.alerts.send_alert(Alert(
                ...
                message=(
                    f"Orphaned: {orphaned_records}, Gaps: {gap_count}, "
                    f"Duplicate active: {duplicate_active}"
                ),
                source_agent=self.name,
