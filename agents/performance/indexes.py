...
class IndexMixin:
    ...
    def detect_unused_indexes(self, project_id: str, branch_id: str, days: int = 7) -> dict:
        ...
        return {
            ...
            "total_reclaimable_mb": sum(row.get("index_size_bytes", 0) / (1024 * 1024) for row in rows),
            "recommendations": [
                {"index": r.index_name, "table": r.table_name, "confidence": r.confidence, "impact": r.estimated_impact}
                for r in recommendations
            ],
        }

    def detect_bloated_indexes(self, project_id: str, branch_id: str, threshold: float = 2.0) -> dict:
    ...
    def run_full_index_analysis(self, project_id: str, branch_id: str) -> dict:
        ...
        if total_issues > 0:
            self.emit_event(
                EventType.INDEX_RECOMMENDATION,
                {
                    "project_id": project_id,
                    "branch_id": branch_id,
