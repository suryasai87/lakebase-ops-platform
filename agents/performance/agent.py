...
class PerformanceAgent(MetricsMixin, IndexMixin, MaintenanceMixin, OptimizationMixin, BaseAgent):
    ...
    def register_tools(self) -> None:
        ...
        self.register_tool(
            ...
            self.persist_pg_stat_statements,
            "Capture pg_stat_statements to Delta (every 5 min)",
            schedule="*/5 * * * *",
        )
        ...
        self.register_tool(
            ...
            self.detect_unused_indexes,
            "Find indexes with idx_scan=0 for 7+ days",
            schedule="0 * * * *",
        )
        self.register_tool(
            "detect_bloated_indexes",
            self.detect_bloated_indexes,
            "Find indexes with bloat ratio > 2.0x",
            schedule="0 * * * *",
        )
        self.register_tool(
            "detect_missing_indexes",
            self.detect_missing_indexes,
            "Find tables with seq_scan >> idx_scan",
            schedule="0 * * * *",
        )
        self.register_tool(
            "detect_duplicate_indexes",
            self.detect_duplicate_indexes,
            "Find overlapping column sets",
            schedule="0 * * * *",
        )
        self.register_tool(
            "detect_missing_fk_indexes",
            self.detect_missing_fk_indexes,
            "Find unindexed foreign keys",
            schedule="0 * * * *",
        )
        self.register_tool(
            "run_full_index_analysis",
            self.run_full_index_analysis,
            "Complete index health analysis (all checks)",
            schedule="0 * * * *",
        )
        ...
        self.register_tool(
            ...
            self.check_txid_wraparound_risk,
        ...
        self.register_tool(
            ...
            self.tune_autovacuum_parameters,
            "Dynamically adjust per-table autovacuum thresholds",
            schedule="0 3 * * *",
        )
        ...
        self.register_tool(
            ...
            self.analyze_slow_queries_with_ai,
        ...
        self.register_tool(
            ...
            self.forecast_capacity_needs,
            "ML-based prediction of storage and compute needs",
            schedule="0 4 * * 0",
        )
